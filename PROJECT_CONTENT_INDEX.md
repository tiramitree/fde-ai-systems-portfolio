...
1. `secure-enterprise-knowledge-copilot`
   ...
   - Core behaviors: admin-only ingestion, connector-style source sync, source ACL snapshot enforcement, permission drift evidence, retrieval filtering, citations, abstention, prompt-injection detection, traces, audit logs, golden evals.
...
- `docs/stale_profile_pin_evidence_examples.md`: stale profile-pin evidence examples for old profile screenshots, wrong pinned repositories, stale profile caches, social-preview confusion, and private account UI crops.
...
- `docs/release_asset_checksum_mismatch_examples.md`: release asset checksum mismatch examples for stale local replay artifacts, changed generated artifact hashes, missing public attachments, wrong release tag checksums, and post-publish checksum drift.
...
- `docs/stale_launch_feedback_claim_examples.md`: stale launch-feedback claim examples for old stars/forks, stale public comments, private feedback summaries, analytics screenshots, launch-post reposts, and evidence-boundary review.
