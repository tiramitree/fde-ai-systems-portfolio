...
| README screenshots do not silently drift from the UI source. | `docs/visual_assets_manifest.json`, `scripts/check_visual_asset_manifest.py`, `docs/visual_asset_hygiene.md` | `python -B scripts/dev.py visual-assets` | Visual assets are treated as release artifacts: screenshot hashes, dimensions, and frontend source hashes must line up. |
...
| Container release config is controlled. | `scripts/check_container_release.py`, `docker-compose.yml`, project `.dockerignore` files, `docs/container_release_hygiene.md` | `python -B scripts/dev.py container-release` | Docker packaging is treated as release-facing code: ports, health checks, commands, env defaults, and ignored local state must stay aligned. |
