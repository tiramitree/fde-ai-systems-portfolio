...
- `/js/scenarioEditor.js` is served for browser-local scenario draft and copy/export behavior.
...
This is not a full production web security posture. It is a local-demo contract that proves the UI is served intentionally: no remote bundle, no accidental source-file exposure, stable content types, keyboard trace navigation assets, browser-local scenario draft copy assets, and basic browser safety headers.
