...
2. Extend the current admin-only ingestion, ACL-snapshot source sync, and GitHub read connector contracts into file upload, broader external connectors, source user/group membership sync, connector checkpoint recovery, and live deletion/prune verification against external source APIs.
