...
2. Extend the current admin-only text/file-like ingestion, ACL-snapshot source sync, and GitHub read connector contracts into multipart uploads, broader external connectors, source user/group membership sync, connector checkpoint recovery, and live deletion/prune verification against external source APIs.
