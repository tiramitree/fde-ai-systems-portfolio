...
- `POST /api/connectors/source-bundle/sync` adds an allowlisted checked-in source bundle connector. It reads only public synthetic files under `data/source_bundles/<bundle>`, rejects unsafe bundle names and path traversal, maps manifest ACL snapshots into source sync ingestion jobs, supports opt-in prune, records manifest and payload hashes, and writes `source_bundle_synced` audit evidence without exposing raw file bodies through status or job summaries.
...
2. Extend the current admin-only text/file-like ingestion, ACL-snapshot source sync, source bundle connector, source-group permission checks, and GitHub read connector contracts into multipart uploads, broader external connectors, source user/group membership sync, connector checkpoint recovery, and live deletion/prune verification against external source APIs.
