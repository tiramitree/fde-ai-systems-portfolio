...
- `GET /api/connectors/status` adds an admin-only operator status boundary derived from ingestion jobs. It reports connector health, latest cursor, document/chunk counts, ACL drift, prior dead letters, and recovered status without exposing raw source bodies.
...
2. Extend the current admin-only ingestion, ACL-snapshot source sync, and GitHub read connector contracts into file upload, broader external connectors, source user/group membership sync, deletion/prune handling, and checkpoint recovery.
