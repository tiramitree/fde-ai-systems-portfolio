from __future__ import annotations

import subprocess
import sys
import time
from pathlib import Path
from urllib.request import urlopen

from public_safety_scan import run_scan


ROOT = Path(__file__).resolve().parents[1]
SERVICES = [
    {
        "name": "secure-enterprise-knowledge-copilot",
        "path": ROOT / "secure-enterprise-knowledge-copilot",
        "port": 8765,
        "health": "http://127.0.0.1:8765/api/health",
    },
    {
        "name": "regulated-customer-operations-agent",
        "path": ROOT / "regulated-customer-operations-agent",
        "port": 8770,
        "health": "http://127.0.0.1:8770/api/health",
    },
    {
        "name": "ai-reliability-incident-console",
        "path": ROOT / "ai-reliability-incident-console",
        "port": 8780,
        "health": "http://127.0.0.1:8780/api/health",
    },
]

REQUIRED_FILES = [
    "README.md",
    "PROJECT_CONTENT_INDEX.md",
    "CHANGELOG.md",
    ".gitattributes",
    "LICENSE",
    "CONTRIBUTING.md",
    "SECURITY.md",
    "ROADMAP.md",
    "CODE_OF_CONDUCT.md",
    ".github/CODEOWNERS",
    ".github/dependabot.yml",
    ".github/pull_request_template.md",
    ".github/workflows/ci.yml",
    ".github/workflows/nightly-regression.yml",
    ".github/ISSUE_TEMPLATE/bug_report.md",
    ".github/ISSUE_TEMPLATE/eval_case.md",
    ".github/ISSUE_TEMPLATE/feature_request.md",
    ".github/ISSUE_TEMPLATE/config.yml",
    "docs/final_demo_runbook.md",
    "docs/final_readiness_report.md",
    "docs/demo_report.md",
    "docs/demo_replay_artifact.md",
    "docs/demo_state_presets.json",
    "docs/container_release_hygiene.md",
    "docs/visual_asset_hygiene.md",
    "docs/visual_assets_manifest.json",
    "docs/project_case_notes.md",
    "docs/production_upgrade_notes.md",
    "docs/postgres_pgvector_adapter_design.md",
    "docker-compose.postgres.yml",
    "infra/postgres/migrations/001_core.sql",
    "infra/postgres/migrations/002_project1_denied_evidence_count.sql",
    "infra/postgres/init/000_project1_runtime_roles.sql",
    "infra/postgres/init/003_project1_runtime_grants.sql",
    "infra/postgres/seeds/001_project1_demo.sql",
    "docs/otel_trace_export.md",
    "docs/model_runtime_configuration.md",
    "docs/model_gateway_safety.md",
    "docs/launch_assets_hygiene.md",
    "docs/observability_integrity.md",
    "docs/threat_model.md",
    "docs/ai_governance_control_registry.json",
    "docs/industrial_true_production_gap_plan_2026_06_07.md",
    "docs/scenario_data_integrity.md",
    "docs/trace_to_eval_workflow.md",
    "docs/reviewed_eval_dataset_ledger.json",
    "docs/error_hygiene.md",
    "docs/supply_chain_security.md",
    "docs/architecture_boundaries.md",
    "docs/workflow_security.md",
    "docs/frontend_integrity.md",
    "docs/fresh_clone_experience.md",
    "docs/runtime_ui_contracts.md",
    "docs/api_contracts.md",
    "docs/public_release_audit.md",
    "docs/differentiation_strategy.md",
    "docs/technical_review_playbook.md",
    "docs/system_design_deep_dive.md",
    "docs/portfolio_evidence_matrix.md",
    "docs/adr_0001_local_first_portfolio.md",
    "docs/adr_0002_model_is_not_security_boundary.md",
    "docs/adr_0003_eval_state_isolated_from_demo_state.md",
    "docs/adr_0004_verification_runtime_state_isolation.md",
    "docs/github_branch_protection.json",
    "docs/github_repository_settings.md",
    "docs/community_backlog.md",
    "docs/github_initial_issues.md",
    "docs/github_labels.json",
    "docs/post_publish_checklist.md",
    "docs/published_repository_status.md",
    "docs/case_study_secure_enterprise_knowledge_copilot.md",
    "docs/case_study_regulated_customer_operations_agent.md",
    "docs/demo_video_script.md",
    "docs/demo_recording_checklist.md",
    "docs/star_growth_plan.md",
    "docs/launch_copy_pack.md",
    "docs/reviewer_perspective_checklist.md",
    "docs/github_release_commands.md",
    "docs/github_release_notes_v0.1.0.md",
    "docs/maintainer_review_policy.md",
    "docs/pr_review_runbook.md",
    "docs/pr_review_security.md",
    "docs/assets/github-preview.png",
    "docs/assets/github-preview.svg",
    "docs/assets/architecture-overview.svg",
    "docs/assets/demo-walkthrough.gif",
    "docs/assets/secure-knowledge-copilot-screenshot.png",
    "docs/assets/secure-knowledge-copilot-mobile.png",
    "docs/assets/regulated-ops-agent-screenshot.png",
    "docs/assets/regulated-ops-agent-mobile.png",
    "docs/assets/ai-reliability-incident-console-screenshot.png",
    "docs/assets/ai-reliability-incident-console-mobile.png",
    "scripts/dev.py",
    "local_auth_tokens.py",
    "local_http_limits.py",
    "local_request_governance.py",
    "scripts/check_auth_boundary.py",
    "scripts/check_request_body_limits.py",
    "scripts/check_request_correlation.py",
    "scripts/check_request_governance.py",
    "scripts/check_architecture_boundaries.py",
    "scripts/check_workflow_security.py",
    "scripts/check_model_gateway_safety.py",
    "scripts/check_openai_live_mode.py",
    "scripts/check_otel_collector_handoff.py",
    "scripts/check_postgres_migrations.py",
    "scripts/check_postgres_compose.py",
    "scripts/check_project1_postgres_runtime.py",
    "scripts/check_project1_retrieval_metrics.py",
    "scripts/check_project1_parser_quality_contract.py",
    "scripts/check_project1_source_scan_contract.py",
    "scripts/generate_postgres_seed.py",
    "scripts/check_observability_integrity.py",
    "scripts/check_threat_model.py",
    "scripts/check_ai_governance_controls.py",
    "scripts/check_trace_to_eval.py",
    "scripts/check_reviewed_eval_ledger.py",
    "scripts/check_trace_redaction.py",
    "scripts/check_scenario_data_integrity.py",
    "scripts/check_error_hygiene.py",
    "scripts/check_claim_consistency.py",
    "scripts/check_container_release.py",
    "scripts/check_docker_runtime.py",
    "scripts/check_launch_assets.py",
    "scripts/check_community_issue_pack.py",
    "scripts/check_visual_asset_manifest.py",
    "scripts/summarize_visual_asset_diff.py",
    "scripts/refresh_visual_assets.py",
    "scripts/check_frontend_integrity.py",
    "scripts/check_fresh_clone_experience.py",
    "scripts/check_runtime_ui_contracts.py",
    "scripts/check_api_documentation.py",
    "scripts/check_dependency_surface.py",
    "scripts/check_demo_state_presets.py",
    "scripts/public_safety_scan.py",
    "scripts/check_public_assets.py",
    "scripts/check_github_readiness.py",
    "scripts/check_repository_governance.py",
    "scripts/check_api_contracts.py",
    "scripts/check_runtime_latency_budget.py",
    "scripts/check_runtime_state_isolation.py",
    "scripts/configure_github_launch.py",
    "scripts/manage_community_issues.py",
    "scripts/community_issue_pack.py",
    "scripts/generate_final_readiness_report.py",
    "scripts/check_pr_review_policy.py",
    "scripts/review_open_prs.py",
    "scripts/replay_demo.py",
    "scripts/export_demo_replay_artifact.py",
    "scripts/export_eval_csv.py",
    "scripts/export_traces_otel.py",
    "scripts/export_trace_eval_candidates.py",
    "scripts/trace_redaction.py",
    "scripts/post_publish_check.py",
    "secure-enterprise-knowledge-copilot/src/copilot/api.py",
    "secure-enterprise-knowledge-copilot/.dockerignore",
    "secure-enterprise-knowledge-copilot/README.md",
    "secure-enterprise-knowledge-copilot/docs/technical_review_notes.md",
    "secure-enterprise-knowledge-copilot/web/js/api.js",
    "secure-enterprise-knowledge-copilot/web/js/app.js",
    "secure-enterprise-knowledge-copilot/web/js/dom.js",
    "secure-enterprise-knowledge-copilot/web/js/renderers.js",
    "regulated-customer-operations-agent/src/ops_agent/api.py",
    "regulated-customer-operations-agent/.dockerignore",
    "regulated-customer-operations-agent/README.md",
    "regulated-customer-operations-agent/docs/technical_review_notes.md",
    "regulated-customer-operations-agent/web/js/api.js",
    "regulated-customer-operations-agent/web/js/app.js",
    "regulated-customer-operations-agent/web/js/dom.js",
    "regulated-customer-operations-agent/web/js/renderers.js",
    "ai-reliability-incident-console/README.md",
    "ai-reliability-incident-console/app.py",
    "ai-reliability-incident-console/Dockerfile",
    "ai-reliability-incident-console/.dockerignore",
    "ai-reliability-incident-console/data/seed_state.json",
    "ai-reliability-incident-console/data/eval_cases.json",
    "ai-reliability-incident-console/docs/architecture.md",
    "ai-reliability-incident-console/docs/threat_model.md",
    "ai-reliability-incident-console/docs/demo_script.md",
    "ai-reliability-incident-console/docs/technical_review_notes.md",
    "ai-reliability-incident-console/docs/implementation_status.md",
    "ai-reliability-incident-console/scripts/run_eval.py",
    "ai-reliability-incident-console/src/reliability_console/api.py",
    "ai-reliability-incident-console/src/reliability_console/storage.py",
    "ai-reliability-incident-console/src/reliability_console/triage.py",
    "ai-reliability-incident-console/src/reliability_console/evals.py",
    "ai-reliability-incident-console/web/index.html",
    "ai-reliability-incident-console/web/js/api.js",
    "ai-reliability-incident-console/web/js/app.js",
    "ai-reliability-incident-console/web/js/dom.js",
    "ai-reliability-incident-console/web/js/renderers.js",
]


def run(args: list[str]) -> tuple[bool, str]:
    result = subprocess.run(args, cwd=ROOT, text=True, capture_output=True)
    output = (result.stdout + "\n" + result.stderr).strip()
    return result.returncode == 0, output


def healthy(url: str) -> bool:
    try:
        with urlopen(url, timeout=2) as response:
            return response.status == 200
    except Exception:
        return False


def wait_for_health(url: str, seconds: int = 15) -> bool:
    for _ in range(seconds):
        if healthy(url):
            return True
        time.sleep(1)
    return False


def start_service(service: dict) -> subprocess.Popen | None:
    if healthy(service["health"]):
        print(f"{service['name']} already healthy on port {service['port']}")
        return None

    print(f"Starting {service['name']} on port {service['port']}")
    return subprocess.Popen(
        [
            sys.executable,
            "-B",
            "app.py",
            "--reset",
            "--port",
            str(service["port"]),
        ],
        cwd=service["path"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.STDOUT,
        creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0),
    )


def check_required_files() -> list[str]:
    failures = []
    for rel_path in REQUIRED_FILES:
        if not (ROOT / rel_path).exists():
            failures.append(f"missing required file: {rel_path}")
    return failures


def main() -> int:
    failures = []
    failures.extend(check_required_files())
    failures.extend(run_scan())
    started: list[subprocess.Popen] = []

    command_checks = [
        ("architecture", [sys.executable, "-B", "scripts/check_architecture_boundaries.py"]),
        ("auth-boundary", [sys.executable, "-B", "scripts/check_auth_boundary.py"]),
        ("request-body-limits", [sys.executable, "-B", "scripts/check_request_body_limits.py"]),
        ("request-correlation", [sys.executable, "-B", "scripts/check_request_correlation.py"]),
        ("request-governance", [sys.executable, "-B", "scripts/check_request_governance.py"]),
        ("workflow-security", [sys.executable, "-B", "scripts/check_workflow_security.py"]),
        ("model-gateway-safety", [sys.executable, "-B", "scripts/check_model_gateway_safety.py"]),
        ("observability", [sys.executable, "-B", "scripts/check_observability_integrity.py"]),
        ("runtime-latency-budget", [sys.executable, "-B", "scripts/check_runtime_latency_budget.py"]),
        ("runtime-state-isolation", [sys.executable, "-B", "scripts/check_runtime_state_isolation.py"]),
        ("reviewed-eval-ledger", [sys.executable, "-B", "scripts/check_reviewed_eval_ledger.py"]),
        ("trace-redaction", [sys.executable, "-B", "scripts/check_trace_redaction.py"]),
        ("threat-model", [sys.executable, "-B", "scripts/check_threat_model.py"]),
        ("governance-controls", [sys.executable, "-B", "scripts/check_ai_governance_controls.py"]),
        ("scenario-data", [sys.executable, "-B", "scripts/check_scenario_data_integrity.py"]),
        ("demo-presets", [sys.executable, "-B", "scripts/check_demo_state_presets.py"]),
        ("error-hygiene", [sys.executable, "-B", "scripts/check_error_hygiene.py"]),
        ("assets", [sys.executable, "-B", "scripts/check_public_assets.py"]),
        ("visual-assets", [sys.executable, "-B", "scripts/check_visual_asset_manifest.py"]),
        ("visual-asset-diff", [sys.executable, "-B", "scripts/summarize_visual_asset_diff.py"]),
        ("frontend", [sys.executable, "-B", "scripts/check_frontend_integrity.py"]),
        ("ui-contracts", [sys.executable, "-B", "scripts/check_runtime_ui_contracts.py"]),
        ("api-docs", [sys.executable, "-B", "scripts/check_api_documentation.py"]),
        ("dependency-surface", [sys.executable, "-B", "scripts/check_dependency_surface.py"]),
        ("postgres-compose", [sys.executable, "-B", "scripts/check_postgres_compose.py"]),
        ("postgres-migrations", [sys.executable, "-B", "scripts/check_postgres_migrations.py"]),
        ("postgres-runtime", [sys.executable, "-B", "scripts/check_project1_postgres_runtime.py"]),
        ("retrieval-metrics", [sys.executable, "-B", "scripts/check_project1_retrieval_metrics.py"]),
        ("parser-quality", [sys.executable, "-B", "scripts/check_project1_parser_quality_contract.py"]),
        ("source-scan", [sys.executable, "-B", "scripts/check_project1_source_scan_contract.py"]),
        ("postgres-seed", [sys.executable, "-B", "scripts/generate_postgres_seed.py", "--check"]),
        ("governance", [sys.executable, "-B", "scripts/check_repository_governance.py"]),
        ("community-issues", [sys.executable, "-B", "scripts/check_community_issue_pack.py"]),
        ("launch-assets", [sys.executable, "-B", "scripts/check_launch_assets.py"]),
        ("pr-policy", [sys.executable, "-B", "scripts/check_pr_review_policy.py"]),
        ("contracts", [sys.executable, "-B", "scripts/check_api_contracts.py"]),
        ("health", [sys.executable, "-B", "scripts/check_health.py"]),
        ("smoke", [sys.executable, "-B", "scripts/smoke_test_demo_flows.py"]),
        ("trace-to-eval", [sys.executable, "-B", "scripts/export_trace_eval_candidates.py"]),
        ("trace-to-eval-check", [sys.executable, "-B", "scripts/check_trace_to_eval.py"]),
        ("otel-traces", [sys.executable, "-B", "scripts/export_traces_otel.py"]),
        ("otel-collector-handoff", [sys.executable, "-B", "scripts/check_otel_collector_handoff.py"]),
        ("report", [sys.executable, "-B", "scripts/generate_demo_report.py"]),
        ("evals", [sys.executable, "-B", "scripts/run_all_evals.py"]),
        ("replay-artifact", [sys.executable, "-B", "scripts/export_demo_replay_artifact.py"]),
        ("claims", [sys.executable, "-B", "scripts/check_claim_consistency.py"]),
        ("container-release", [sys.executable, "-B", "scripts/check_container_release.py"]),
    ]
    try:
        for service in SERVICES:
            process = start_service(service)
            if process is not None:
                started.append(process)

        for service in SERVICES:
            if not wait_for_health(service["health"]):
                failures.append(f"service did not become healthy: {service['name']}")

        for name, command in command_checks:
            ok, output = run(command)
            print(f"\n=== {name} ===")
            print(output)
            if not ok:
                failures.append(f"command failed: {name}")
    finally:
        for process in started:
            process.terminate()
        for process in started:
            try:
                process.wait(timeout=5)
            except subprocess.TimeoutExpired:
                process.kill()

    if failures:
        print("\nQuality gate failed:")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("\nQuality gate passed.")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
