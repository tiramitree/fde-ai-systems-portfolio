...
REQUIRED_PRODUCTION_UPGRADE_POINTER = [
    ...
    "| PostgreSQL and pgvector | [PostgreSQL And pgvector Adapter Design](docs/postgres_pgvector_adapter_design.md) and `infra/postgres/migrations/001_core.sql`. | Preserve permission checks before retrieval or side effects, keep eval state isolated, and run `python -B scripts/dev.py postgres-migrations`, `python -B scripts/dev.py scenario-data`, and `python -B scripts/dev.py quality`. |",
