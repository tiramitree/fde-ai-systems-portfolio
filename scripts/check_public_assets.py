...
REQUIRED_README_CAPTIONS = [
    "Desktop: role-aware knowledge access, visible documents, admin source intake, eval gate, and trace/audit surfaces for permission-aware RAG.",
...
REQUIRED_POST_PUBLISH_READINESS = [
...
REQUIRED_RELEASE_PAGE_READINESS = [
...
def check_post_publish_readiness() -> list[str]:
    readme = ROOT / "README.md"
    if not readme.exists():
        return ["missing README.md"]
    text = readme.read_text(encoding="utf-8")
    failures = []
    for expected in REQUIRED_POST_PUBLISH_READINESS:
        if expected not in text:
            failures.append(f"README.md: missing post-publish readiness entry: {expected}")
    return failures
...
def check_release_page_readiness() -> list[str]:
    readme = ROOT / "README.md"
    if not readme.exists():
        return ["missing README.md"]
    text = readme.read_text(encoding="utf-8")
    failures = []
    for expected in REQUIRED_RELEASE_PAGE_READINESS:
        if expected not in text:
            failures.append(f"README.md: missing release page readiness entry: {expected}")
    return failures
...
def main() -> int:
    ...
    failures.extend(check_release_page_readiness())
